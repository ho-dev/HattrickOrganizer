...
public class SQLDialog extends JDialog implements ActionListener {
    ...
    public void refresh() {
        ...
        if(matcher.find()){
            ...
            try {
                int rows = DBManager.instance().getAdapter().executePreparedUpdate(getTextArea().getText());
    ...
    private Object[][] getValues() {
        ...
        try
        {
            ...
            ResultSet rs = DBManager.instance().getAdapter().executePreparedQuery(select + " " + sql);
            rs.last();
            rowCount = rs.getRow();
            rs.beforeFirst();
