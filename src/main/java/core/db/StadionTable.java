...
final class StadionTable extends AbstractTable {
	...
	protected void initColumns() {
		columns = new ColumnDescriptor[]{
			...
			ColumnDescriptor.Builder.newInstance().setColumnName("AusbauKosten").setGetter(p -> ((Stadium) p).getExpansionCosts()).setSetter((p, v) -> ((Stadium) p).setExpansionCosts((Integer) v)).setType(Types.INTEGER).isNullable(true).build(),
