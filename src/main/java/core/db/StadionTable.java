...
final class StadionTable extends AbstractTable {
	...
	protected void initColumns() {
		columns = new ColumnDescriptor[]{
			...
			ColumnDescriptor.Builder.newInstance().setColumnName("AusbauKosten").setGetter(p -> ((Stadium) p).getExpansionCosts()).setSetter((p, v) -> ((Stadium) p).setExpansionCosts((int) v)).setType(Types.INTEGER).isNullable(false).build(),
