...
final class StadionTable extends AbstractTable {
	...
	protected void initColumns() {
		columns = new ColumnDescriptor[]{
			...
			ColumnDescriptor.Builder.newInstance().setColumnName("AusbauKosten").setGetter(p -> ((Stadium) p).getExpansionCosts().getSwedishKrona()).setSetter((p, v) -> ((Stadium) p).setExpansionCosts((AmountOfMoney) v)).setType(Types.DECIMAL).isNullable(false).build(),
