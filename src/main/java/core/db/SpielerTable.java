...
final class SpielerTable extends AbstractTable {
	...
	protected void initColumns() {
		columns = new ColumnDescriptor[]{
				...
				ColumnDescriptor.Builder.newInstance().setColumnName("Gehalt").setGetter((p) -> ((Player) p).getWage()).setSetter((p, v) -> ((Player) p).setWage(new AmountOfMoney((int) v))).setType(Types.INTEGER).isNullable(false).build(),
