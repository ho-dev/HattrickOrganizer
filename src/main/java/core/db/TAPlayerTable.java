...
final class TAPlayerTable extends AbstractTable {
	...
	protected void initColumns() {
		columns = new ColumnDescriptor[]{
				...
				ColumnDescriptor.Builder.newInstance().setColumnName("SALARY").setGetter((p) -> ((PlayerInfo) p).getSalary().getSwedishKrona()).setSetter((p, v) -> ((PlayerInfo) p).setSalary(new AmountOfMoney((long) v))).setType(Types.INTEGER).isNullable(true).build(),
