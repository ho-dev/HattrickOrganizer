...
public class TrainingManager {
	...
	private List<TrainingPerWeek> pastTrainings;         // used to populate training history, no match information => Created at initilization
    ...
    private TrainingManager() {
		...
		pastTrainings =  DBManager.instance().getTrainingList();

		// Create recent training history from other tables in database

		Instant previousTrainingDate = pastTrainings.stream().map(t -> t.getTrainingDate()).max(Instant::compareTo).get();
	...
	public List<TrainingPerWeek> getPastTrainings() {
